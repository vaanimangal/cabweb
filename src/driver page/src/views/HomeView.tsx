...
import type { DriverStatus, RideRequest, ActiveTrip } from '../types';
...
export function HomeView({
  status, onToggleStatus, todayEarnings, todayTrips, weeklyTotal, pushToast,
}: {
  status: DriverStatus;
  onToggleStatus: () => void;
  ...
  const handleAccept = (r: RideRequest) => {
    ...
    setActiveTrip({
      id: r.id, passengerName: r.passengerName, pickupAddress: r.pickupAddress,
      destinationAddress: r.destinationAddress, fare: r.estimatedFare,
      distanceKm: r.tripDistanceKm, etaMinutes: r.estimatedMinutes, status: 'to_pickup',
    });
    pushToast('success', `Ride accepted — navigating to ${r.pickupAddress}`);
  };

  const handleReject = (r: RideRequest) => {
    setRequests((cur) => cur.filter((x) => x.id !== r.id));
  ...
  const mapStatus: 'idle' | 'to_pickup' | 'in_trip' = activeTrip ? activeTrip.status : 'idle';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <section className="lg:col-span-3 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Radio size={18} className={online ? 'text-red-500 animate-ping-slow' : 'text-ink-400'} />
              <h2 className="text-lg font-extrabold text-ink-900">Live Ride Requests</h2>
            </div>
            <span className="chip bg-ink-100 text-ink-600">{online ? `${requests.length} incoming` : 'Offline'}</span>
          </div>
          ...
          {online && requests.length > 0 ? (
            ...
            !activeTrip && (
              <div className="card p-10 text-center">
                ...
                <p className="font-bold text-ink-800">{online ? 'Waiting for ride requests' : 'You are offline'}</p>
        ...
        <section className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-extrabold text-ink-900">Navigation</h2>
            <span className="chip bg-gold-50 text-gold-700"><Sparkles size={13} /> Live route</span>
          </div>
...
function StatusBanner({ online, onToggle }: { online: boolean; onToggle: () => void }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl p-5 sm:p-6 transition-colors duration-500 ${
      online ? 'bg-gradient-to-br from-red-600 to-red-700' : 'bg-gradient-to-br from-ink-900 to-ink-800'
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="relative flex w-3 h-3">
              {online && <span className="absolute inline-flex h-full w-full rounded-full bg-white/70 animate-pulse-ring" />}
              <span className={`relative inline-flex rounded-full h-3 w-3 ${online ? 'bg-white' : 'bg-red-500'}`} />
            </span>
            <p className="text-xs font-bold uppercase tracking-wider text-white/70">{online ? 'You are online' : 'You are offline'}</p>
          </div>
          <h2 className="mt-1.5 text-2xl font-extrabold tracking-tight">
            {online ? 'Ready to receive rides' : 'Go online to start earning'}
          </h2>
        ...
        <button
          ...
          className={`btn btn-lg tap shrink-0 font-extrabold text-base px-8 animate-toggle-pop border-2 ${
            online ? 'bg-white text-red-600 border-white hover:bg-ink-100 shadow-lg' : 'bg-red-500 text-white border-red-500 hover:bg-red-600 shadow-glowRed'
          }`}
          aria-pressed={online}

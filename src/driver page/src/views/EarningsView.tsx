...
import type { EarningsTrip } from '../types';
...
export function EarningsView() {
  ...
  const [filter, setFilter] = useState<'all' | 'cash' | 'card' | 'wallet'>('all');
    ...
    <div className="space-y-5">
      ...
      <div className="card p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2 overflow-x-auto -mx-1 px-1 pb-1 sm:pb-0">
            {(['all', 'cash', 'card', 'wallet'] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`btn btn-md tap shrink-0 capitalize ${filter === f ? 'bg-ink-900 text-white' : 'bg-ink-100 text-ink-700 hover:bg-ink-200'}`}
              >
                {f === 'all' ? 'All' : f}
              </button>
            ))}
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-ink-100 flex items-center justify-between">
          ...
          <span className="text-sm text-ink-500 font-semibold">{filtered.length} trips</span>
        </div>
...
function SummaryCard({
  label, value, prefix = '', suffix = '', decimals = 0, icon: Icon, tone,
}: {
  label: string;
  value: number;
  prefix?: string;
  ...
  return (
    <div className="card p-4 sm:p-5">
      ...
      <p className="text-xs text-ink-500 font-semibold mt-0.5">{label}</p>
    </div>
  );
}

function TripRow({ trip, index }: { trip: EarningsTrip; index: number }) {
  const pay = payMeta[trip.paymentMethod];
  const PayIcon = pay.icon;
  return (
    <li className="px-4 sm:px-5 py-4 hover:bg-ink-50/60 transition-colors animate-fade-in" style={{ animationDelay: `${Math.min(index * 0.03, 0.4)}s` }}>
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="font-bold text-ink-900 truncate">{trip.passengerName}</p>
            <span className={`chip ${pay.color} hidden sm:inline-flex`}>
              <PayIcon size={11} />
              {pay.label}
            </span>
          </div>
          <p className="text-xs text-ink-500 truncate mt-0.5">{trip.pickup} → {trip.dropoff}</p>
          <p className="text-[11px] text-ink-400 font-medium mt-0.5">
            {trip.date} · {trip.time} · {trip.distanceKm} km · {trip.durationMin} min
          </p>
        </div>
        <div className="text-right shrink-0">
          <p className="font-extrabold text-ink-900">+${trip.fare.toFixed(2)}</p>
          <p className="text-[11px] text-ink-400 font-semibold mt-0.5">{pay.label}</p>
        </div>
      </div>
    </li>
  );

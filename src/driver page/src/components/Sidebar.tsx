    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-5 h-16 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              <p className="font-extrabold tracking-tight text-[15px]">GEO RIDES</p>
              <p className="text-[11px] text-white/50 font-medium">GO GEO, GO PLACES</p>
            ...
            return (
              <button
                ...
                className={`group w-full flex items-center gap-3 px-3 h-12 rounded-xl text-sm font-semibold transition-all duration-200 tap ${
                  active ? 'bg-red-500 text-white shadow-glowRed' : 'text-white/70 hover:text-white hover:bg-white/10'
                }`}
        ...
        <div className="p-3">
          ...
          >
            <img src={driverProfile.avatarUrl} alt="" className="w-11 h-11 rounded-xl object-cover ring-2 ring-white/10" />
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold truncate">{driverProfile.name}</p>
              <p className="text-xs text-white/50 truncate">{driverProfile.vehiclePlate} · {driverProfile.vehicleModel.split(' ').slice(0, 2).join(' ')}</p>
            </div>
          </button>
        </div>
      </aside>
